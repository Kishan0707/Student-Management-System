...
const Toast = {
  ...
  show(message, type = 'info', duration = 3000) {
    ...
    toast.style.cssText = `border-left-color:${colors[type]}`;
    toast.innerHTML = `
      <span style="color:${colors[type]}">${iconMap[type]}</span>
...
const Store = {
  ...
  update(key, id, updates) {
    const arr = this.get(key).map(i => i.id === id ? { ...i, ...updates } : i);
    this.set(key, arr);
  },
  remove(key, id) {
...
class Paginator {
  ...
  renderControls(containerId, onPageChange) {
    ...
    html += `</div><span class="text-xs text-slate-400 ml-2">Page ${this.page} of ${total} · ${this.data.length} records</span>`;
    el.innerHTML = html;
  }
}
...
const Skeleton = {
  ...
  cards(n = 4) {
    return Array(n).fill(0).map(() =>
      `<div class="stat-card"><div class="skeleton h-4 w-24 mb-3"></div><div class="skeleton h-8 w-16 mb-2"></div><div class="skeleton h-3 w-32"></div></div>`
    ).join('');
  }
};
...
function seedDemoData() {
  ...
  const firstNames = ['Aiden','Bella','Carlos','Diana','Ethan','Fiona','George','Hannah','Ivan','Julia','Kevin','Laura','Marcus','Nina','Oscar','Priya','Quinn','Rachel','Samuel','Tara','Uma','Victor','Wendy','Xander','Yara','Zoe'];
